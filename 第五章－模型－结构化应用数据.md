...
### Reserved  eld names
A few  eld names are reserved to be used by the ORM:  
...
The following  elds are automatically created on new models, unless the _log_ access=False model attribute is set:  
...
The preceding code shows the basic syntax for these  elds, setting the related model and the  eld's title string. The convention for relational  eld names is to append _id or _ids to the  eld names, for to one and to many relations, respectively.  
...
### Referencing  elds using dynamic relations
...
We can use it to add a To-do Task  eld, Refers to, that can either refer to a User or a Partner:  
...
You can see that the  eld de nition is similar to a Selection  eld, but here the selection list holds the models that can be used. On the user interface, the user will  rst pick a model from the list, and then pick a record from that model.  
