...
Odoo模块和应用经常被提及，但是它们之间到底有什么区别呢？模块用来构建Odoo应用的区块。模块可以添加或者修改Odoo的功能。通过在目录中包含一个明显的描述符文件（称作`__openerp__.py`）,以及剩余的文件实现模块功能。有时候，模块可以称作“附加组件”。应用不同于常规的模块，从功能上来说，应用提供了核心功能，并为其他模块提供了功能添加和一些选项。应用对功能区提供了核心元素，围绕着其他模块的功能添加，比如账单或者人力资源。所以，应用常常在Odoo的Apps菜单中高亮显示。  
...
紧接着第一章的介绍，我们开始Odoo开发，我们应该服务器应该位于/odoo-dev/odoo/。为了保持整洁，我们在主机的自定义模块中创建一个新的目录。  

```bash
$ mkdir ~/odoo-dev/custom-addons
```

An Odoo module is a directory containing an `__openerp__.py` descriptor  le. This is still a legacy from when Odoo was named OpenERP, and in the future is expected to become `__odoo__.py`.  

Odoo模块是一个包含描述符`__openerp__.py`文件的目录。这名字来源自Odoo还被称作OpenERP的时候，在未来会被称作`__odoo__.py`。  

It also needs to be Python importable, so it must also have an `__init__.py` file.  

它也需要导入Python，所以它必须拥有一个`__init__.py`文件。  

The module's directory name will be its technical name. We will use `todo_app` for it. The technical name must be a valid Python identi er: it should begin with a letter and can only contain letters, numbers, and the underscore character. The following commands create the module directory and create an empty `__init__.py`  le in it:  

模块的目录名称会成为它技术层面的名称。我们要使用`todo_app`这个名字。技术名称必须是一个有效的Python识别符：它应该以字母开始，而且只能包含字母、数字、和下划线。下面的命令创建了模块目录，然后在模块目录后创建了一个空文件`__init__.py`：  
...
接下来，我们需要创建一个描述符文件。它应该只包含一个可能拥有一打属性的Python字典，其中只有name属性是必须的。

We should now add an `__openerp__.py`  le alongside the `__init__.py`  le with the following content:  
...
depends属性拥有一个所需模块的列表。Odoo会在该模块安装时自动地安装依赖。这并不是必写的属性，不过这里建议你写上去。如果不需要特殊的依赖，我们应该使用特殊的base模块作为依赖。你应该用心地保证所有的依赖都是正确的，否则在清理数据库（处理缺失的依赖）时模块可能会安装失败，或者是在其他的必需模块向后载入时发生载入错误。就我们的应用而言，我们想依赖mail模块，因为，这是一个添加到消息顶层菜单中的模块，而去我们也想让自己的菜单出现在这里。  
...
现在我们已经拥有新的模块了，尽管它很小，我们也想在Odoo中使用它。  
...
The `--save` option saves the options you used in a con g  le. This spares you from repeating them the next time you restart the server: just run ./odoo.py and the last saved options will be used.  
...
开发模块是一个交互式的过程，你需要改变需要应用的源文件并在Odoo中展示它。  
...
很多情况下本操作都是通过升级模块来完成的：在本地模块列表中查看模块，因为已经安装了，所以你会看到一个可用的升级按钮。  
...
不过，当只是改变Python中的代码的话，升级并没有生效。与升级模块相反，应用服务器的重启是必需的。  
...
(customer, supplier, and so on.). A model has a list of attributes and can also de ne its speci c business.  
...
Some consider it good practice to keep the Python  les for models inside a models subdirectory. For simplicity we won't be following that here, so let's create a todo_model.py  le in the todo_app module main directory.  
...
第一行是一个告诉Python解释器该文件使用的是UTF－8的特殊标记，因此它可以用来处理非－ASCII字符。我们不再使用除此之外的任何东西，不过为了安全起见最好还是使用它。  

The second line makes available the models and fields objects from the Odoo core.  

第二行从Odoo内核构建可用的models和fields对象。  

The third line declares our new model. It's a class derived from models.Model. The next line sets the `_name` attribute de ning the identi er that will be used throughout Odoo to refer to this model. Note that the actual Python class name is meaningless to the other Odoo modules. The `_name` value is what will be used as an identifier.  

第三行生命我们的新模型。这是一个派生自models.Model的类。接下来的行设置

Notice that this and the following lines are indented. If you're not familiar with
Python you should know that this is important: indentation de nes a nested code
block, so these four line should all be equally indented.  

The last three lines de ne the model's  elds. It's worth noting that name and active are names of special  elds. By default Odoo will use the name  eld as the record's title when referencing it from other models. The active  eld is used to inactivate records, and by default only active records will be shown. We will use it to clear away completed tasks without actually deleting them from the database.  

最后三行定义了模型的字段。

Right now, this  le is not yet used by the module. We must tell Odoo to load it with the module in the __init__.py  le. Let's edit it to add the following line:  

现在，这个文件还没有被模块使用。我们必需在__init__.py文件中告诉Odoo使用模块载入这个文件。让我们编辑这个文件，并添加下面的行：  
...
We can also see some additional  elds we didn't declare. These are the  ve reserved  elds Odoo automatically adds to any model. They are as follows:  
...
现在，我们拥有了一个存储数据的模型，让我们将它显示在用户接口上吧。  
...
<data>
 ...
 If everything went right, this will let us con rm that the model and our  elds
  </data>
</openerp>
```

The user interface, including menu options and actions, is stored in database tables.
The XML  le is a data  le used to load those de nitions into the database when the module is installed or upgraded. This is an Odoo data  le, describing two records to add to Odoo:  
...
`<menuitem>`通过被定义之前，调用动作action_todo_task来定义“消息”菜单（通过mail.mail_feeds识别）。sequence让我们设置菜单选项的顺序。  

Now we need to tell the module to use the new XML data  le. That is done in the `__openerp__.py` file using the data attribute. It de nes the list of  les to be loaded by the module. Add this attribute to the descriptor's dictionary:  
...
Views should be de ned for models to be exposed to the users, but Odoo is nice
enough to do that automatically if we don't, so we can work with our model right
away, without having any form or list views de ned yet.  
...
>### Tips
>In case an upgrade fails because of an XML error, don't panic! Comment out the last edited XML portions, or remove the XML  le from __openerp__.py, and repeat the upgrade. The server should start correctly. Now read the error message in the server log carefully—it should point you to where the problem is.  

## Creating views – form, tree, and search
As we have seen, if no view is de ned, Odoo will automatically generate basic views to get you going. But surely you would like to de ne the module views yourself, so that's what we'll do next.  
...
All views are stored in the database, in the ir.model.view model. To add a view in a module, we declare a <record> element describing the view in an XML  le that will be loaded into the database when the module is installed.  
...
The most important attribute is arch, containing the view de nition. Here we say it's a form, and it contains three  elds, and we chose to make the active  eld read only.  
...
Forms can have buttons to run actions. These are able to trigger work ow actions, run Window Actions, such as opening another form, or run Python functions de ned in the model.  
...
The basic attributes for a button are: string with the text to display on the button, the type of action it performs, and the name that is the identi er for that action. The optional class attribute can apply CSS styles, just like in regular HTML. 
...
We can add the following tree view de nition to todo_view.xml:  
...
We have de ned a list with only two columns, name and is_done. We also added a
nice touch: the lines for done tasks (is_done==True) are shown in grey.  

At the top right of the list Odoo displays a search box. The default  elds it searches
for and available prede ned  lters can be de ned by a `<search>` view.  
...
The <field> elements de ne  elds that are also searched when typing in the search box. The <filter> elements add prede ned  lter conditions, using domain syntax that can be selected with a user click.  

## Adding business logic  添加业务逻辑
Now we will add some logic to our buttons. Edit the todo_model.py Python  le to add to the class the methods called by the buttons.
...
As you can see, it simply modi es the is_done  eld, inverting its value. Methods, then, can be called from the client side and must always return something. If they return None, client calls using the XMLRPC protocol won't work. If we have nothing to return, the common practice is to just return the True value.  
...
The message is pretty clear: our new model has no access rules, so it can't be used by anyone other than the admin super user. As a super user the admin ignores data access rules, that's why we were able to use the form without errors. But we must  x this before other users can use it.  
...
This information needs to be provided by the module, using a data  le to load the
...
Our module is looking good. Why not add an icon to it to make it look even better? For that we just need to add to the module a static/description/icon.png  le with the icon to use.  
...
Always remember, when adding model  elds, an upgrade is needed. When changing Python code, including the manifest  le, a restart is needed. When changing XML or CSV  les, an upgrade is needed; also when in doubt, do both: upgrade the modules and restart the server.
